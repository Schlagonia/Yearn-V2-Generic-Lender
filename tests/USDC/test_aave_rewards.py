...
def test_no_emissions(
    ...
    cloned_strategy.setDebtThreshold(
        strategy.debtThreshold(), {"from": vault.governance()}
    )
    cloned_strategy.setProfitFactor(
        strategy.profitFactor(), {"from": vault.governance()}
    )
    cloned_strategy.setMaxReportDelay(
        strategy.maxReportDelay(), {"from": vault.governance()}
    )
    ...
    vault.updateStrategyDebtRatio(
        vault.withdrawalQueue(0), 0, {"from": vault.governance()}
    )
    vault.updateStrategyDebtRatio(
        vault.withdrawalQueue(1), 0, {"from": vault.governance()}
    )
    vault.addStrategy(
        cloned_strategy, debt_ratio, 0, 2 ** 256 - 1, 500, {"from": vault.governance()}
    )
