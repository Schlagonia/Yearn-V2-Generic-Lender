...
@pytest.fixture
def v3Plugin(strategist, GenericAaveV3, strategy, wavax, router, router2):
    v3Plugin = strategist.deploy(
        GenericAaveV3, strategy, wavax.address, router, router2, "AaveV3", True
    )
...
@pytest.fixture
def v3PluginUsdc(strategist, GenericAaveV3, strategyUsdc, wavax, router, router2):
    v3PluginUsdc = strategist.deploy(
        GenericAaveV3, strategyUsdc, wavax.address, router, router2, "AaveV3", True
    )
...
@pytest.fixture
def pluggedStrategyUsdc(strategyUsdc, v3PluginUsdc, gov):
...
@pytest.fixture
def pluggedVaultUsdc(pluggedStrategyUsdc, vaultUsdc, gov):
    vaultUsdc.addStrategy(
        pluggedStrategyUsdc, 10_000, 0, 2 ** 256 - 1, 1_000, {"from": gov}
    )
